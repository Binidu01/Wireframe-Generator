...
const WireframeTool: React.FC = () => {
  ...
  const [currentCanvasState, setCurrentCanvasState] = useState<ImageData | null>(null);
  ...
  const getCanvasCoords = useCallback((e: React.MouseEvent, canvas: HTMLCanvasElement) => {
  ...
  const edgeDetect = useCallback((ctx: CanvasRenderingContext2D, width: number, height: number) => {
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      if (e.key.toLowerCase() === 'y' || (e.shiftKey && e.key === 'Z')) { e.preventDefault(); redo(); }
      if (e.key.toLowerCase() === 'x')     { e.preventDefault(); replaceWithCrossbox(); }
    };
  ...
  const selectionText = selection
    ? `Selection: ${Math.round(selection.w)}×${Math.round(selection.h)} at (${Math.round(selection.x)}, ${Math.round(selection.y)})`
    : 'No selection';
        ...
        <div className="bg-gradient-to-r from-blue-500 to-cyan-400 p-8 text-white text-center relative overflow-hidden">
          ...
          <div className="relative z-10">
            ...
            <p className="text-lg opacity-90">
              Transform your UI designs into clean wireframe sketches
        ...
        <div className="p-8 bg-white flex justify-center items-center min-h-[60vh] relative">
          ...
          {!img && (
            <div className="absolute text-center text-gray-500 animate-pulse p-16">
              ...
              <p className="text-lg">Upload an image to start creating wireframes</p>
            </div>
          )}
          {isLoading && (
